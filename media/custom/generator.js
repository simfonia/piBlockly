...
// The finish function assembles all the code into a complete Arduino sketch.
Blockly.Arduino.finish = function(code) {
  // Indent the code for loop()
  code = '  ' + code.replace(/\n/g, '\n  ');
  code = code.replace(/\n\s+$/, '\n'); // Remove trailing spaces on newlines
  ...
  var setups = [];
  for (var setupCode in Blockly.Arduino.setups_) {
    setups.push(Blockly.Arduino.setups_[setupCode]);
  }
  var setupCode = 'void setup() {\n  ' + setups.join('\n  ') + '\n}\n';

  // Assemble definitions (includes, global variables, functions)
  var definitions = [];
  for (var defCode in Blockly.Arduino.definitions_) {
    definitions.push(Blockly.Arduino.definitions_[defCode]);
  }
  var definitionsCode = definitions.join('\n') + '\n\n';

  // Combine all parts
  return definitionsCode + setupCode + code;
};

// Generator Functions for custom blocks

// Initializes Blocks
Blockly.Arduino['initializes_setup'] = function(block) {
